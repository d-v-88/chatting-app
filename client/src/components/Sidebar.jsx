    ...
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? 'max-md:hidden': ''}`}>
      <div className="pb-5">
         <div className="flex justify-between items-center">
             <img src={assets.qonvo} alt="logo" className="max-w-20" />
             ...
             key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
              ...
              <div className="flex flex-col leading-5">
                  <p>{user.fullName}</p>
                  {
                    ...
                    : <span className="tetx-neutral-400 text-sm"> Offline</span>
                  }
              </div>
      ...
      {/* <button className="absolute bottom-5 pl-60 transform -translate-x-1/2 bg-gradient-to-r from-purple-400 to-voilet-600 text-white border-none text-sm py-2 px-20 font-light rounded-full cursor-pointer">
        Group chat +
      </button> */}
